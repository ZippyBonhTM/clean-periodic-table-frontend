...
type ResolveMoleculeEditorLayoutStylesOptions = {
  activeView: 'editor' | 'structural' | 'simplified' | 'stick';
  ...
  paletteSearchRailHeight: number;
...
export default function resolveMoleculeEditorLayoutStyles({
  activeView,
  bottomNoticeHeight,
  canvasFrameSize,
  componentCount,
  ...
  topControlsHeight,
  topOverlayHeight,
}: ResolveMoleculeEditorLayoutStylesOptions) {
  ...
  const topOverlayClassName = isLandscapeCompactCanvas ? 'absolute inset-x-2 top-2 z-30' : 'absolute inset-x-3 top-3 z-30';
  const toolRailCollapsedWidthClassName = isLandscapeCompactCanvas ? 'w-10' : 'w-12';
  const toolRailExpandedWidthClassName = isLandscapeCompactCanvas ? 'w-[min(56vw,8.5rem)]' : 'w-[min(72vw,224px)] sm:w-52';
